...
- (void)viewDidLoad {
    [super viewDidLoad];
    if ([FFUserModel currentUser].mobile) {
...
- (void)initUnbindPhoneView {
    ...
    NSMutableString *phone = [NSMutableString stringWithFormat:@"%@",[FFUserModel currentUser].mobile];
...
- (void)respondsToNext {
    ...
    BOX_START_ANIMATION;
    [FFUserModel userBindingPhoneNumber:self.phoneNumber.text Code:self.securityCode.text completion:^(NSDictionary *content, BOOL success) {
        BOX_STOP_ANIMATION;
        if (success) {
...
- (void)unbindPhone {
    ...
    [FFUserModel userUnbindingPhoneNumber:[FFUserModel currentUser].mobile Code:self.securityCode.text completion:^(NSDictionary *content, BOOL success) {
        if (success) {
...
- (UILabel *)unbindLabel {
    if (!_unbindLabel) {
        ...
        _unbindLabel.textColor = [UIColor lightGrayColor];
...
- (UIButton *)unbindButton {
    if (!_unbindButton) {
        ...
        _unbindButton.backgroundColor = NAVGATION_BAR_COLOR;
