//
//  FFTransferServerViewController.m
//  FF_185_Game_Box
//
...
#import "FFTransferServerViewController.h"
...
@interface FFTransferServerViewController ()<FFRebateSelectViewDelegate, UIScrollViewDelegate, FFApplyTransferViewDelegate,UITableViewDelegate>
...
@implementation FFTransferServerViewController
...
- (void)refreshNewData {
    _currentPage = 1;
    [FFUserModel transferGameListWithPage:[NSString stringWithFormat:@"%ld",_currentPage] Completion:^(NSDictionary *content, BOOL success) {
        if (success) {
...
- (UIBarButtonItem *)rightButton {
    if (!_rightButton) {
        _rightButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"New_transfer_?"] style:(UIBarButtonItemStyleDone) target:self action:@selector(respondsToRightButton)];
    }
    return _rightButton;
}
...
- (UILabel *)remindLabel {
    if (!_remindLabel) {
        ...
        syLog(@"%lf",kSCREEN_WIDTH);
//        [_remindLabel sizeToFit];
//        _remindLabel.center = CGPointMake(kSCREEN_WIDTH / 2, _remindLabel.center.y);
    }
    return _remindLabel;
}
