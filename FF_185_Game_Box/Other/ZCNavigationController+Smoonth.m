//
//  ZCNavigationController+Smoonth.m
//  众彩生活
//
...
@interface FFNavigationController()<UINavigationBarDelegate>


@end

@implementation FFNavigationController (Smoonth)

+ (void)initialize {
    if (self == [FFNavigationController self]) {
...
//    if ([topVC isKindOfClass:[ZCViewController class]]) {
...
- (void)setNavBarBgAlpha:(CGFloat)navBarBgAlpha {
    ...
    dispatch_async(dispatch_get_main_queue(),^{
          [(FFNavigationController*)self.navigationController setNeedsNavigationBackgroundAlpha:navBarBgAlpha];
    });
}
